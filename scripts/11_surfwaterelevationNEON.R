...
elevation_files <- googledrive::drive_ls(path = elevation, type = "csv")
elevation_files <- elevation_files[grepl("elev", elevation_files$name), ]

# Create an empty list to store the cleaned data frames
elevation_list <- lapply(seq_along(elevation_files$name), function(i) {
  googledrive::drive_download(
    file = elevation_files$id[i],
    path = paste0("googledrive/", elevation_files$name[i]),
    overwrite = TRUE
  )
  
  # Read the CSV file
  read.csv(paste0("googledrive/", elevation_files$name[i]), header = TRUE)
})

# Assign names to the list elements based on the file names
names(elevation_list) <- elevation_files$name
